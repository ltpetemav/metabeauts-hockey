...
export function TurnPanel({
  ...
  if (phase === 'RPS') {
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
        <h3 className="text-white font-bold text-lg mb-3">✊ Rock-Paper-Scissors</h3>
        <p className="text-gray-400 text-sm mb-3">
          {viewingPlayer === 'player1' ? 'Player 1' : 'Player 2'}: Choose your move!
          Winner gets possession.
        </p>
        {thisPlayerChose ? (
          <div className="text-green-400">✅ Choice locked! Waiting for opponent...</div>
        ) : (
          <div className="flex gap-3">
            {(['rock', 'paper', 'scissors'] as RPSChoice[]).map(choice => (
              <button
                ...
                className="flex-1 py-3 px-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-white font-bold border border-gray-600 hover:border-gray-400 transition-all hover:scale-105"
              >
                <div className="text-2xl">{choice === 'rock' ? '✊' : choice === 'paper' ? '🖐' : '✌️'}</div>
  ...
  if (phase === 'POSSESSION_START' || phase === 'LINE_CHANGE_OFFENSIVE' || phase === 'LINE_CHANGE_DEFENSIVE') {
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
        <h3 className="text-white font-bold text-lg mb-2">🔄 Line Change?</h3>
        <p className="text-gray-400 text-sm mb-3">
  ...
  if (phase === 'OFFENSIVE_DRAW') {
    if (!isOffensivePlayer) {
      return (
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
          <div className="text-gray-400">🎲 Opponent is drawing an action card...</div>
        </div>
      );
    }
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
        <h3 className="text-white font-bold text-lg mb-2">🎲 Draw Your Card</h3>
        <p className="text-gray-400 text-sm mb-3">
        ...
        {gameState.can_shoot ? (
          ...
          <div className="bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 mb-3 text-gray-400 text-sm">
            ⛔ canShoot = FALSE — Must Pass or Skate first before shooting
          </div>
        )}
        <button
          onClick={onDrawCard}
          className="w-full py-3 rounded-xl bg-green-700 hover:bg-green-600 text-white font-bold text-lg transition-all hover:scale-105"
        >
  ...
  if (phase === 'DEFENSIVE_RESPONSE') {
    if (isOffensivePlayer) {
      return (
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
          <h3 className="text-white font-bold mb-2">You drew:</h3>
          {drawn_card && (
            <div className="flex items-center gap-3">
              <ActionCardUI cardType={drawn_card.card_type} size="lg" />
              <div>
                <div className="text-white font-bold text-xl">{drawn_card.card_type}</div>
                {drawn_card.is_trait && (
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
        <h3 className="text-white font-bold text-lg mb-2">🛡️ Select Your Defense</h3>
        <p className="text-gray-400 text-sm mb-3">
          Choose a card from {defensiveBeaut.name}&apos;s pile to counter the offense.
          You have <span className="text-white font-bold">{availDef.length} cards</span> available.
        </p>
        {drawn_card && (
          <div className="mb-3 bg-gray-800 rounded-lg px-3 py-2">
            ...
            <span className="text-gray-400 text-xs ml-2">(you cannot see this in real game)</span>
          </div>
        )}
        {availDef.length === 0 ? (
  ...
  if (phase === 'TRAIT_WINDOW') {
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
        <h3 className="text-white font-bold text-lg mb-2">✨ Trait Activation Window</h3>

        {/* Show both cards about to resolve */}
        <div className="flex gap-4 justify-center mb-3">
          {drawn_card && (
            <div className="text-center">
              <div className="text-xs text-orange-400 mb-1">⚔️ Offense</div>
              <ActionCardUI cardType={drawn_card.card_type} size="sm" />
              <div className="text-xs text-gray-400 mt-1">{drawn_card.card_type}</div>
            </div>
          )}
          <div className="flex items-center text-gray-500 text-xl">VS</div>
          {defensive_selected_card && (
            <div className="text-center">
              <div className="text-xs text-blue-400 mb-1">🛡️ Defense</div>
        ...
        {hasForcedTrait && !alreadyActivated ? (
          <div>
            <div className="bg-purple-900/50 border border-purple-600 rounded-lg px-3 py-2 mb-3">
              <div className="text-purple-300 font-bold">✨ {myTrait!.trait_name} available!</div>
              <div className="text-purple-400 text-xs mt-1">
                Type: {myTrait!.trait_type} | {isOffensivePlayer ? 'Offensive' : 'Defensive'}
              </div>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => onActivateTrait(viewingPlayer, myTrait!.trait_name)}
                className="flex-1 py-2 rounded-lg bg-purple-700 hover:bg-purple-600 text-white font-bold"
              >
          ...
          <div>
            ...
            {bothCardsReady && (
              <button
                ...
                className="w-full py-3 rounded-xl bg-red-700 hover:bg-red-600 text-white font-bold text-lg transition-all hover:scale-105"
              >
                ⚡ Resolve Action!
              </button>
            )}
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <div className="text-xs text-gray-400 mb-1">Send to bench (select from ice):</div>
          <div className="flex flex-wrap gap-1">
            {iceBeauts.map(b => (
              ...
              >
                {b.name.replace('MetaBeauts #', '#')} ({b.position})
              </button>
            ))}
          </div>
        </div>
        <div>
          <div className="text-xs text-gray-400 mb-1">Bring from bench (select):</div>
          <div className="flex flex-wrap gap-1">
            {benchBeauts.map(b => (
              <button
                key={b.id}
      ...
      <div className="flex gap-2">
        {selectedIce && selectedBench && (
          <button
            ...
            className="flex-1 py-2 rounded-lg bg-blue-700 hover:bg-blue-600 text-white font-bold"
          >
            ✅ Confirm Line Change
          </button>
        )}
